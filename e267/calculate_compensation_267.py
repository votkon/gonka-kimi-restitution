#!/usr/bin/env python3
"""
Epoch 267 Restitution Calculator — ComputeGroupCap underpayment

Issue:
  All Kimi participants entered the epoch, but the Kimi model group's total weight
  exceeded the ComputeGroupCap (cap_factor × N-1 total network weight). The chain
  applied a uniform scale factor to every Kimi participant's ValidationWeight.Weight,
  cutting it to ~28% of the true earned weight.

  As a result, the chain's reward formula — which uses ConfirmationWeight rescaled
  through Weight — was bounded by the capped Weight, causing systematic underpayment
  to every Kimi contributor.

  No delegation compensation is needed for this epoch: all Kimi operators entered
  the epoch group, so no delegators were forced into ModeNone.

Cap parameters (from chain params at height 4120752):
  cap_factor             = 0.75
  N-1 total weight       = 335,159  (epoch 266 total ValidationWeight)
  Kimi group cap         = 251,369
  Kimi raw weight        = 899,487
...
Eligibility:
  ...
  confirmation_weight present but rewards = 0) are excluded: they did not successfully
  complete the epoch and should not receive restitution for it.
...
def main():
    ...
    # Kimi contributors are the only eligible compensees, but the denominator
    # uses all participants — the pool is shared across the whole epoch
    ...
    if excluded:
        ...
        for a in excluded:
            cw_str = "no conf_w" if "confirmation_weight" not in next(x for x in kimi_vw if x["member_address"]==a) else "conf_w present but rewards=0"
            print(f"  {a}  ({cw_str})")
        print()
    ...
    for x in kimi_vw:
        ...
        # Only compensate healthy participants: must have confirmed work AND received rewards
    ...
    with open(out(f"compensation_{EPOCH}.json"), "w") as f:
        json.dump({
            ...
            "cap_factor":                      0.75,
            "n1_total_weight":                 335159,
            "kimi_group_cap":                  251369,
