#!/usr/bin/env python3
"""
Epoch 268 Restitution Calculator — ComputeGroupCap underpayment

Issue:
  Same as epoch 267: all Kimi participants entered the epoch, but the Kimi model
...
Cap parameters (from chain params at height 4136143):
  cap_factor             = 0.75
  N-1 total weight       = epoch 267 total ValidationWeight (fetched from chain)
...
def main():
    ...
    # uses for reward distribution (GRC precedent from e247 audit). Keeps cap reductions
    # from redistributing unclaimed shares to remaining participants.
